...
class WhiskeyDisk
  ...
  def if_task_defined(task, cmd)
    %(rakep=`#{env_vars} rake -P` && if [[ `echo "${rakep}" | grep #{task}` != "" ]]; then #{cmd}; fi )
  end

  def safe_branch_checkout(path, my_branch)
    %(cd #{path} && git checkout -b #{my_branch} origin/#{my_branch} || git checkout #{my_branch} origin/#{my_branch} || git checkout #{my_branch})
  end
  ...
  def refresh_checkout(path, repo_branch)
    enqueue "cd #{path}"
    enqueue "git fetch origin +refs/heads/#{repo_branch}:refs/remotes/origin/#{repo_branch} #{unless debugging?
                                                                                                '&>/dev/null'
                                                                                              end}"
  ...
  def run_rake_task(path, task_name)
    ...
    enqueue(if_file_present("#{setting(:deploy_to)}/Rakefile",
                            if_task_defined(task_name,
                                            "#{env_vars} rake #{if debugging?
                                                                  '--trace'
                                                                end} #{task_name} to=#{setting(:environment)}")))
  end
  ...
  def run_script(script)
    ...
    enqueue(%(cd #{setting(:deploy_to)}; echo "Running post script..."; #{env_vars} bash #{if debugging?
                                                                                             '-x'
                                                                                           end} #{build_path(script)}))
  end
  ...
  def snapshot_git_revision
    ...
    enqueue %(ml=\`git log -1 --pretty=format:%H\`)
  end
