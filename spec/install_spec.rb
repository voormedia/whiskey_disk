...
describe 'the plugin install.rb script' do
  before do
    stub!(:puts).and_return(true)
  end

  it 'displays the content of the plugin README file' do
    stub!(:readme_contents).and_return('README CONTENTS')
    should.receive(:puts).with('README CONTENTS')
    do_install
  end
  ...
  describe 'readme_contents' do
    it 'works without arguments' do
      do_install
      -> { readme_contents }.should.not.raise(ArgumentError)
    end

    it 'accepts no arguments' do
      do_install
      -> { readme_contents(:foo) }.should.raise(ArgumentError)
    end
